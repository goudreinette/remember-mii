...
void generateRandomPath() {
  ...
  int segmentWidth =
      (endX - startX) /
      (NUM_POINTS + 1);  // Total space divided by number of points
  ...
  for (int i = 0; i < NUM_POINTS; i++) {
    int x =
        prevX + segmentWidth;  // Calculate the next X position, evenly spaced
    // int y = 50; //Random::get(50, 300); // Randomize Y position between 150
    // and 250
    int y = Random::get(50, 300);  // Randomize Y position between 150 and 250

    // Ensure the points are spaced out evenly
    x = constrain(x, startX + 10,
                  endX - 10);     // Constrain to avoid points on edges
  ...
  PathLine endPoint =
      PathLine{.x1 = prevX, .y1 = prevY, .x2 = endX, .y2 = endY};
...
void showElectrocutedAnimation() {
  GRRLIB_DrawImg(170, 130, electrocutedImages[electrocutedFrame / 4 % 2], 0,
                 1.5, 1.5, TRANSPARENT_WHITE);  // Draw a jpeg
  electrocutedFrame++;
}
...
void showMainMenu() {
  ...
  Rect startButtonRect = Rect{
      .x = width / 2 - 75, .y = height / 2 - 25, .width = 150, .height = 50};

  bool hoveringStartButton = GRRLIB_PtInRect(
      startButtonRect.x, startButtonRect.y, startButtonRect.width,
  ...
  if (mousePressed && hoveringStartButton) {
    // Generate a path!
    generateRandomPath();
    // SYS_Report("points: %i\r", pathLines.size()); // Log to check if
    // generation went ok
    ...
    gameOver = false;
    gameWon = false;
    electrocutePlayed = false;  // Reset electrocutePlayed and chillPlayed flags
...
void showAndCheckRestartButton() {
  Rect restartButtonRect = Rect{
      .x = width / 2 - 75, .y = height / 2 + 80, .width = 150, .height = 50};

  bool hoveringRestartButton = GRRLIB_PtInRect(
      restartButtonRect.x, restartButtonRect.y, restartButtonRect.width,
      restartButtonRect.height, mouseX, mouseY);
  int restartButtonColor = hoveringRestartButton ? WHITE : ORANGE;
  int restartTextColor = BLACK;
  GRRLIB_Rectangle(restartButtonRect.x, restartButtonRect.y,
                   restartButtonRect.width, restartButtonRect.height,
                   restartButtonColor, true);
  GRRLIB_Printf(width / 2 - 55, height / 2 + 100, fontTexture, restartTextColor,
                1, "RESTART");
...
void showGameOver() {
  ...
  GRRLIB_Printf(width / 2 - (strlen(menuText) * 16), 100, fontTexture, RED, 2,
                "%s", menuText);
...
void showGameWon() {
  ...
  GRRLIB_Printf(textX - (strlen(menuText) * 16), textY, fontTexture, LIME, 2,
                "%s", menuText);
...
void playGame() {
  ...
  GRRLIB_Printf(width / 2 - (strlen(levelText.c_str()) * 16), 30, fontTexture,
                WHITE, 2, levelText.c_str());

  // Score
  std::string scoreText = std::format("SCORE: {}", score);
  GRRLIB_Printf(width / 2 - (strlen(scoreText.c_str()) * 8), 350, fontTexture,
                WHITE, 1, scoreText.c_str());

  // Time left
  std::string timeLeftText = std::format("{}", remainingTime());
  // SYS_Report("time left: %i\r", timeLeftText);
  GRRLIB_Printf(width / 2 - (strlen(timeLeftText.c_str()) * 8), 400,
                fontTexture, remainingTime() < 10 ? RED : WHITE, 1,
                timeLeftText.c_str());
